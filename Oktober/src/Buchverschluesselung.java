...
public class Buchverschluesselung {

    private final static String book = "Zur ZEIT des Zweiten Weltkriegs waren seine großen Werke Siddhartha und Der Steppenwolf noch verboten. Heute gehört Hermann Hesse zu den bekanntesten deutschen Schriftstellern. Mehr über den Weltveränderer lest ihr hier\n" +
            "Hermann Hesse\n" +
            "\n" +
    ...
    public static void main(String[] args) {
        String textToEncrypt = "java insel";
        String encryptedText = encryption(book, textToEncrypt);
        System.out.println(encryptedText);
        String decryptedText = decryption(book, encryptedText);
        System.out.println(decryptedText);
    }


    private static String encryption(String book, String textToEncrypt) {
        String encryptedText = "";
        int position = 0;
        int lastPosition = 0;
        //System.out.println(textToEncrypt.charAt(i));
        for (int i = 0; i < textToEncrypt.length(); i++) {
            position = book.toLowerCase().indexOf(textToEncrypt.toLowerCase().charAt(i), position);
            //System.out.println(position - lastPosition);
            encryptedText = encryptedText + (position - lastPosition) + " ";
    ...
    private static String decryption(String book, String textToDecrypt) {
        String decryptedText = "";
        int position = 0;
        String[] positions = textToDecrypt.split("[ ]");
        for (int i = 0; i < positions.length; i++) {
            position = position + Integer.parseInt(positions[i]);
            decryptedText = decryptedText + book.toLowerCase().charAt(position);
        }
